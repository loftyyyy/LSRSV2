...
class InventoryController extends Controller
{
    ...
    public function generateCSV(Request $request)
    {
        ...
        $headers = [
            ...
            'Content-Disposition' => 'attachment; filename="inventory_report_'.$reportType.'_'.now()->format('Y-m-d').'.csv"',
            'Pragma' => 'no-cache',
...
fputs($output, chr(0xEF) . chr(0xBB) . chr(0xBF)); // BOM for UTF-8
            ...
            if ($reportType === 'inventory_summary') {
                ...
                foreach ($reportData['by_status'] as $status => $items) {
                    fputcsv($output, [$status, $items->count()]);
                }
                fputcsv($output, []); // Empty row

                fputcsv($output, ['Item Details']);
                fputcsv($output, ['Item ID', 'Name', 'SKU', 'Type', 'Size', 'Color', 'Design', 'Status', 'Variant SKU', 'Rental Price', 'Deposit Amount', 'Image Count']);
            ...
            } elseif ($reportType === 'availability_report') {
                // Availability Report
                fputcsv($output, ['Total Available Items', $reportData['total_available']]);
                fputcsv($output, []); // Empty row

                fputcsv($output, ['Available Items by Type']);
                foreach ($reportData['by_type'] as $type => $items) {
                    fputcsv($output, [$type, $items->count()]);
                }
                fputcsv($output, []); // Empty row

                fputcsv($output, ['Available Items by Size']);
                foreach ($reportData['by_size'] as $size => $items) {
                    fputcsv($output, [$size, $items->count()]);
                }
                fputcsv($output, []); // Empty row

                fputcsv($output, ['Available Item Details']);
                fputcsv($output, ['Item ID', 'Name', 'SKU', 'Type', 'Size', 'Color', 'Design', 'Status', 'Variant SKU', 'Rental Price', 'Deposit Amount', 'Image Count']);

                foreach ($reportData['available_items'] as $item) {
                    fputcsv($output, [
                        $item->item_id,
                        $item->name,
                        $item->sku ?? 'N/A',
            ...
            } elseif ($reportType === 'rental_history') {
                // Rental History Report
                fputcsv($output, ['Total Rentals', $reportData['total_rentals']]);
                fputcsv($output, []); // Empty row

                fputcsv($output, ['Item Details']);
                fputcsv($output, ['Item ID', 'Name', 'SKU', 'Type', 'Size', 'Color', 'Design', 'Status', 'Variant SKU', 'Rental Price', 'Deposit Amount', 'Rental Count']);
            ...
            } elseif ($reportType === 'condition_report') {
                // Condition Report
                fputcsv($output, ['Item Details']);
                fputcsv($output, ['Item ID', 'Name', 'SKU', 'Type', 'Size', 'Color', 'Design', 'Status', 'Variant SKU', 'Rental Price', 'Deposit Amount']);
                ...
                fputcsv($output, []); // Empty row
                fputcsv($output, ['Condition Breakdown']);
                foreach ($reportData['by_condition'] as $condition => $items) {
                    fputcsv($output, [$condition, $items->count()]);
                }
                ...
                fputcsv($output, ['Item Revenue Details']);
                fputcsv($output, ['Item ID', 'Name', 'SKU', 'Type', 'Rental Count', 'Total Revenue', 'Average Revenue']);

                foreach ($reportData['items'] as $itemData) {
                    $item = $itemData['item'];
                    fputcsv($output, [
                        $item->item_id,
                        $item->name,
